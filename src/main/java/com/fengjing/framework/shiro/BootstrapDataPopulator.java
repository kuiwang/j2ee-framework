...
public class BootstrapDataPopulator implements InitializingBean {
  ...
  public void afterPropertiesSet() throws Exception {
    // because we're using an in-memory hsqldb for the sample app, a new one will be created each
    // time the
    ...
    // new Sha256Hash( <password>, <cryptographically strong randomly generated salt> (not the
    // username!) )
    String query =
        "insert into users values ('admin', '" + new Sha256Hash("admin", "admin").toBase64()
            + "' )";
    ...
    query =
        "insert into users values ( 'user', '" + new Sha256Hash("user", "user").toBase64() + "' )";
    jdbcTemplate.execute(query);
    log.debug("����user�û�.");
